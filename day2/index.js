...
//let input[0].indexOf(' ', 6)
//console.log(input[0].indexOf(' ', 0)); // 5
//console.log(input[0].indexOf(' ', 6)); // 8
//console.log(input[0].substring(0, 5)); // 15-19
//console.log(input[0].substring(6, 8)); // k:
//console.log(input[0].substring(9)); // kkkkkkkkkkkkzkkkkkkk

const isValid = (inpStr, flag) => {
  let ws1;
  let ws2;
  let range;
  let rangeMin;
  ...
  ws1 = inpStr.indexOf(' ', 0);
  ws2 = inpStr.indexOf(' ', ws1 + 1);
  range = inpStr.substring(0, ws1);
  char = inpStr.substring(ws1 + 1, ws1 + 2);
  string = inpStr.substring(ws2 + 1);
  rangeMin = range.substring(0, 2);
  rangeMax = range.substring(2);
  charInString = string.split(char).length - 1;

  rangeMaxInt = Math.abs(parseInt(rangeMax, 10));
  rangeMinInt = parseInt(rangeMin, 10);

  //console.log('ws1: ', ws1);
  //console.log('ws2: ', ws2);
  //console.log('range: ', range, range.length);
  //console.log('rangeMin: ', rangeMin, rangeMin.length);
  //console.log('rangeMax: ', rangeMax, rangeMax.length);
  //console.log('rangeMinInt: ', rangeMinInt);
  //console.log('rangeMaxInt: ', rangeMaxInt);
  //console.log('char: ', char, char.length);
  //console.log('string', string, string.length);
  //console.log('charInString', charInString);
  //console.log(rangeMinInt + rangeMaxInt);

  let positionMinChar = string[rangeMinInt - 1];
  let positionMaxChar = string[rangeMaxInt - 1];
  let result;
  //console.log('positionMaxChar: ', positionMaxChar);
  //console.log('positionMinChar: ', positionMinChar);
  switch (flag) {
    case '-r':
      if (charInString >= rangeMinInt && charInString <= rangeMaxInt) {
        result = true;
      } else {
        result = false;
      }
      break;
    case '-p':
      if (positionMinChar == char || positionMaxChar == char) {
        if (positionMinChar != positionMaxChar) {
          result = true;
        }
      } else {
        result = false;
...
const isValidNum = (inp, flag) => {
  let counter = 0;
  inp.forEach((element) => {
    if (isValid(element, flag)) {
      ++counter;
    }
  });
  return counter;
...
//--part two
//console.log(isValid(input[0], '-p'));
console.log(isValidNum(input, '-r'));
//isValidNum(input);
